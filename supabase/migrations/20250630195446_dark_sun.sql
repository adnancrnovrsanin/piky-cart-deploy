/*
  ...
  3. Error Handling
    ...
    - Removed reference to non-existent shared_lists table
*/
...
CREATE POLICY IF NOT EXISTS "Users can manage their own lists"
  ON shopping_lists
  USING (user_id = auth.uid());

-- 2. Collaborators can view lists they have access to
CREATE POLICY IF NOT EXISTS "Collaborators can view lists"
  ON shopping_lists
  FOR SELECT
  USING (
    id IN (
...
CREATE POLICY IF NOT EXISTS "Editors can update lists"
  ON shopping_lists
  FOR UPDATE
  USING (
    id IN (
...
CREATE POLICY IF NOT EXISTS "Users can manage items in their own lists"
  ON list_items
  USING (
    list_id IN (
...
CREATE POLICY IF NOT EXISTS "Collaborators can view list items"
  ON list_items
  FOR SELECT
  USING (
    list_id IN (
...
CREATE POLICY IF NOT EXISTS "Editors can manage list items"
  ON list_items
  FOR INSERT
  WITH CHECK (
    list_id IN (
...
CREATE POLICY IF NOT EXISTS "Editors can update list items"
  ON list_items
  FOR UPDATE
  USING (
    list_id IN (
...
CREATE POLICY IF NOT EXISTS "Editors can delete list items"
  ON list_items
  FOR DELETE
  USING (
    list_id IN (
...
CREATE POLICY IF NOT EXISTS "Users can view collaborators for their lists"
...
CREATE POLICY IF NOT EXISTS "Users can view invitations they've sent"
...
CREATE POLICY IF NOT EXISTS "Users can manage shared links for their lists"
  ON shared_links
  USING (
    list_id IN (
      SELECT id FROM shopping_lists WHERE user_id = auth.uid()
...
CREATE POLICY IF NOT EXISTS "Anyone can view active shared links"
  ON shared_links
  FOR SELECT
